...
export function UploadForm({ uploadedCount, maxCount, onUploadSuccess }: UploadFormProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setStatusMessage({ type: "success", text: MESSAGES["MSG-UI-C001"] });
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>{UI_TEXT.UPLOAD_FORM.DESCRIPTION(maxCount)}</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        {statusMessage && (
          <Alert variant={statusMessage.type === "error" ? "destructive" : "default"}>
            {statusMessage.type === "success" && <CheckCircle2 className="text-green-600 size-4" />}
            {statusMessage.type === "error" && <AlertCircle className="size-4" />}
            {statusMessage.type === "warning" && <AlertCircle className="text-amber-600 size-4" />}
        ...
        {previewUrl && selectedFile && (
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-sm truncate">{UI_TEXT.UPLOAD_FORM.SELECTED_LABEL}{selectedFile.name}</span>
            ...
            <div className="relative overflow-hidden rounded-lg border bg-muted h-64 flex items-center justify-center">
              <img src={previewUrl} alt="選択画像プレビュー" className="max-w-full max-h-full object-contain" />
            </div>
          </div>
        )}
